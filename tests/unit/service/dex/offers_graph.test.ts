...
describe("buildFromOffers()", () => {
  it("builds graph correctly", () => {
    ...
    expect(ledTseData).to.exist;
    expect(ledTseData!.capacity).to.deep.equal(new BigNumber(17));
    expect(ledTseData!.orderBook.getOrders()).to.deep.equal([
...
describe("findPaymentPaths()", () => {
  it("works", () => {
    ...
    expect(paths.LED).to.exist;
    expect(paths.LED.path).to.deep.equal(["MOW"]);
  });
});
